...
func TestSealOpenRoundtrip(t *testing.T) {
	s := testSealer(t)
	now := time.Now()
	in := accessClaims{Subject: "sub-1", Email: "a@b.c", GoogleAccessToken: "ya29.x", ExpiresAt: 42, IssuedAt: now.Unix()}
...
func TestOpenRejects(t *testing.T) {
	s := testSealer(t)
	now := time.Now()
	blob, err := sealBlob(s, accessBlob, accessClaims{Subject: "s", IssuedAt: now.Unix()})
	require.NoError(t, err)
	...
	t.Run("expired blob rejected by spec TTL", func(t *testing.T) {
		stale, err := sealBlob(s, codeBlob, codeClaims{Subject: "s", IssuedAt: now.Add(-2 * codeTTL).Unix()})
		require.NoError(t, err)
...
func TestKeyRotation(t *testing.T) {
	...
	blob, err := sealBlob(oldSealer, refreshBlob, refreshClaims{Subject: "s", GoogleRefreshToken: "1//r", IssuedAt: now.Unix()})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.Equal(t, "1//r", rc.GoogleRefreshToken)
...
func TestSealOpenProperty(t *testing.T) {
	...
	rapid.Check(t, func(t *rapid.T) {
		in := codeClaims{
			...
			Domain:             rapid.String().Draw(t, "hd"),
			ClientID:           rapid.String().Draw(t, "cid"),
			RedirectURI:        rapid.String().Draw(t, "ru"),
			CodeChallenge:      rapid.String().Draw(t, "cc"),
			Scopes:             rapid.SliceOfN(rapid.String(), 0, 4).Draw(t, "scopes"),
			GoogleAccessToken:  rapid.String().Draw(t, "gat"),
			GoogleRefreshToken: rapid.String().Draw(t, "grt"),
			GoogleExpiry:       rapid.Int64().Draw(t, "gexp"),
		...
		if len(in.Scopes) == 0 {
			in.Scopes = nil // omitempty drops empty slices
		}
		assert.Equal(t, in, out)
	})
}
