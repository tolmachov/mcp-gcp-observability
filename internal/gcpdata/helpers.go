...
// safeInt32 converts int to int32, clamping to math.MaxInt32 to prevent overflow.
func safeInt32(n int) int32 {
	if n > math.MaxInt32 {
		return math.MaxInt32
	}
	return int32(n) //nolint:gosec // overflow is guarded by the check above
}
...
// formatTimestamp converts a proto timestamp to RFC3339 string.
...
func formatLatency(d *durationpb.Duration) string {
	...
	return fmt.Sprintf("%.1fms", float64(dur.Milliseconds()))
}
