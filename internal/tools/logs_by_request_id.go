...
func RegisterLogsByRequestID(s *mcp.Server, client *gcpclient.Client) {
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		if err != nil {
			...
			return errResult(fmt.Sprintf("Failed to query logs by request ID: %v. Verify the request_id is correct. Use logs.find_requests to discover valid request IDs.", err)), nil, nil
		}
